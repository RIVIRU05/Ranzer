...
RANZER CLI
Usage:
  python3 -m ranzer.cli start --dirs /path/to/folder
  python3 -m ranzer.cli status
  python3 -m ranzer.cli log [--limit 20] [--severity HIGH]
  python3 -m ranzer.cli scan --file /path/to/file
  python3 -m ranzer.cli export --format json --output alerts.json
"""
...
PID_FILE   = "/tmp/ranzer.pid"
STATE_FILE = "/tmp/ranzer_state.json"
...
def cmd_start(args):
    ...
    if not dirs:
        print("[!] No directories specified. Use --dirs /path/to/dir")
        sys.exit(1)
    for d in dirs:
...
def cmd_status(args):
    ...
    # Load state file (written by both CLI and GUI)
    state = None
    ...
    try:
        ...
        # Alert counts - check logs/ subfolder first, then fallback
...
def _resolve_log_file() -> str:
    """Find the current event log using the state file, falling back to cwd."""
    candidates = []
    if os.path.exists(STATE_FILE):
        try:
            with open(STATE_FILE) as f:
            ...
            candidates.append(os.path.join(log_dir, "logs"))   # dir itself for glob fallback
        except Exception:
            pass
...
def main():
    ...
    p.add_argument("--dirs", required=True, help="Comma-separated directories to monitor")
    p.add_argument("--threshold", type=float, default=7.5, help="Entropy threshold (default 7.5)")
