...
# PyInstaller spec for RANZER - one-folder bundle.
# Run via: build_deb.sh  (do NOT run directly unless you know what you're doing)
...
a = Analysis(
    ...
    pathex=["."],
    binaries=[],
    # Assets land at the bundle ROOT (next to the executable).
    # _gui_resource() in ranzer/gui/__init__.py resolves them there at runtime.
    datas=[
        ("ranzer/gui/logo.png",          "."),
...
exe = EXE(
    ...
    strip=False,
    upx=True,
    console=True,   # keep console so CLI commands print output
