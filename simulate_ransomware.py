...
On Linux, keeping the file handle open during write is especially important
because RANZER's /proc/[pid]/fd scan runs while the file is still open.
...
def _write_random(path: str, size_kb: int):
    ...
    The pause while the handle is open lets /proc/[pid]/fd catch this PID."""
...
def main():
    ...
    if not args.new_only:
        ...
        # Adds .enc extension - triggers IN_MOVED_FROM/IN_MOVED_TO inotify events;
        # also fires the honey file renamed event if a honey file was renamed.
        ...
        # Creates a text file - triggers IN_CREATE (low entropy, but creates event).
        ...
        try:
            ...
            _print(f"Phase 3 - Note drop failed: {e}")

        _print("")

    # ── Phase 4: Create new high-entropy files ────────────────────────────────
    # Triggers rapid write rate + entropy detection + /proc/[pid]/fd PID scan.
